...
def generate_file_count(pathvalue1,final_datas,file_qcc_log_filename_path,exclude_list):
    for src_dir, dirs, files in os.walk(pathvalue1, topdown=True):
        ...
        for dir_ in dirs:
            ...
            # get folder size of images
            folder_size = get_folder_size(current_src)

            # Collect data
            #final_data = (file_count, parent_dir, grandparent_dir, current_src)
            final_data = (parent_dir, current_dir, file_count, folder_size, current_src)

            final_datas.append(final_data)
    df = pd.DataFrame(final_datas, columns=header)
...
def generate_site_list(pathvalue1,islands,sites,exclude_list, output_sitelist_path):
    print('--- Islandand site list CSV Started ----')
    islands = []
    sites = []
    ...
    # Get the island and site information
    island_site_df = generate_site_list(pathvalue1)
    # Save the DataFrame to a CSV file in the source directory
    island_site_df.to_csv(output_sitelist_path, index=False)
...
def main():
    #import urser inputs 
    args = parse_args()
    pathvalue1 = args.SfM_Folder
    generate_site_list_flag = args.generate_site_list
    generate_file_count_flag = args.generate_file_count
    #generate_file_list_flag = args.generate_file_list
    #check_duplicates_flag = args.check_duplicates
    
    # And modify the exclude_list in the main function:
    #exclude_list_file_path = os.path.join(pathvalue1, "directories_list.csv")
    #exclude_list = load_exclude_list(exclude_list_file_path)
    
    # ------------ Setup Site List Variables ------------
    site_list_file_info_header = ['ISLAND', 'SITE']
    #site_list_final_datas = []
    output_sitelist_path = os.path.join(pathvalue1, 'island_site_info.csv')
   # site_list_file_log_filename = "02_site_list_Info.csv"
   # site_list_file_log_filename_path = os.path.join(pathvalue1, site_list_file_log_filename)
    
     # ------------ Setup File Count Variables ------------
    file_count_header = ['ISLAND', 'SITE', 'FILE_COUNT', 'FOLDER_SIZE', 'DIRECTORY']
    final_datas = []
 #   file_qcc_log_filename = "03_file_count_Info.csv"
  #  file_qcc_log_filename_path = os.path.join(pathvalue1, file_qcc_log_filename)
    exclude_list = ['MISC', 'SITE','_SITE', 'Products', 'ISLAND','ISLANDCODE']  # Add directory names to exclude here
# Prepare the log file
    current_datetime = datetime.now().strftime("%m_%d_%Y_%H%M")
# Update output csv file name if desired
    file_qcc_log_filename = "_SfM_File_count_" + current_datetime + ".csv"
# choose path for which the csv file will be saved
    file_qcc_log_filename_path = os.path.join(pathvalue1, file_qcc_log_filename)

    # ------------ Setup File List Variables ------------
   # file_info_header = ['TEAM','SITE', 'DIRECTORY_PATH', 'FILENAME','FULL_FILE_PATH']
   # file_log_filename = "04_file_list_Info.csv"
   # file_log_filename_path = os.path.join(pathvalue1, file_log_filename)

    #run 
    if generate_file_count_flag:
        #generate_file_count(file_count_header, pathvalue1, file_qcc_log_filename_path)
        generate_file_count(pathvalue1,final_datas,file_qcc_log_filename_path,exclude_list)

    if generate_site_list_flag:
        #generate_site_list(site_list_file_info_header, final_datas, output_sitelist_path)
        generate_site_list(pathvalue1,exclude_list, output_sitelist_path)

   # if generate_file_list_flag:
     #   generate_file_list(file_info_header, pathvalue1, exclude_list, file_log_filename_path)
   # if check_duplicates_flag:
    #    check_duplicates(pathvalue1,exclude_list) 
    # if args.generate_dir_list:
    #     dir_list_file_path = os.path.join(pathvalue1, "directories_list.csv")
    #     generate_dir_list(pathvalue1, dir_list_file_path)
